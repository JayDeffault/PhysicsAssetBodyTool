...
bool FVehiclePhATNativeConvexTool::AddViewportVertexMarker(FString& OutMessage)
{
    ...
    if (LastObservedMarkerPoints.Num() == Points.Num())
    {
        for (int32 PointIndex = 0; PointIndex < Points.Num(); ++PointIndex)
        {
            if (!Points[PointIndex].Equals(LastObservedMarkerPoints[PointIndex], 0.01f))
            {
    ...
    const int32 PreviousSelectedIndex = Points.IsValidIndex(SelectedIndex)
        ? SelectedIndex
        : (Points.Num() > 0 ? Points.Num() - 1 : INDEX_NONE);
    ...
    OutMessage = FString::Printf(TEXT("Added marker vertex %d at %.3f %.3f %.3f. The tool reused the selected/recent marker primitive slot when available so the PhAT transform gizmo can stay on the new marker."), SelectedIndex, NewPoint.X, NewPoint.Y, NewPoint.Z);
    return true;
}
