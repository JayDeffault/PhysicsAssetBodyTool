...
Copy/Paste body settings now preserves target shape transforms when applying common shape settings: boxes, spheres, capsules, tapered capsules and convex elements keep their target location/rotation while dimensions/settings are copied. Paste Transform refreshes the edited body setup and supports tapered capsules and convex transform data in addition to boxes/spheres/capsules.

To use symmetry for **Edit Convex Bodies**: load/select the desired body and `Convex index`, choose the X/Y/Z axis checkboxes in the **Symmetry** row, then either tick **Enable** for live symmetry or click **Apply Symmetry Now** for a one-shot rebuild. **Apply Symmetry Now** does not turn on **Enable**; it rebuilds from the current negative-side source markers every time, so repeated clicks clean/rebuild instead of freezing the old marker count. Symmetry affects only that selected convex index. Once **Enable** is active, moving marker spheres in the PhAT viewport triggers the normal debounced update and then rebuilds the same selected convex from the symmetric marker point cloud; adding a marker while **Enable** is active immediately reapplies symmetry so the new negative-side marker gets a positive-side counterpart. Symmetry is computed one-way on world/component axes: marker points are transformed from the selected body bone space through the Preview Skeletal Mesh reference pose, only points on the negative side of the selected world X/Y/Z axes are used as the source, they are mirrored across world coordinate 0 to the positive side, then transformed back to body-local convex points. Automatic live symmetry rebuilds cap the point cloud to the active symmetry marker count and hide surplus marker slots so marker spheres do not multiply after each move. Untick **Enable** to return to non-mirrored marker editing.
