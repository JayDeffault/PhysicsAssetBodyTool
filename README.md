...
To enable symmetry for **Edit Convex Bodies**: load/select the desired body and `Convex index`, choose the X/Y/Z axis checkboxes in the **Symmetry** row, then tick **Enable** or click **Apply Symmetry Now**. Symmetry affects only that selected convex index. Once enabled, moving marker spheres in the PhAT viewport triggers the normal debounced update and then rebuilds the same selected convex from the symmetric marker point cloud. Symmetry is computed one-way on world/component axes: marker points are transformed from the selected body bone space through the Preview Skeletal Mesh reference pose, only points on the negative side of the selected world X/Y/Z axes are used as the source, they are mirrored across world coordinate 0 to the positive side, then transformed back to body-local convex points. The first symmetry apply records the target marker count; later automatic symmetry rebuilds and repeated **Apply Symmetry Now** presses reuse that count, cap the point cloud, and rebuild the marker spheres, hiding surplus marker slots so marker spheres do not multiply after each move or button press. Untick **Enable** to return to non-mirrored marker editing.
